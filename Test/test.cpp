...
TEST(TestMegaSort, SortDifferent)
{
	...
	EXPECT_NO_THROW(sort(mas, mas+10000, [](int a, int b) { return a < b; }));
...
TEST(TestQuickSort, SortDifferent)
{
	...
	EXPECT_NO_THROW(quickSort(mas, mas+10000, [](int a, int b) { return a < b; }));
	for (int i = 0; i < 10000 - 1; i++)
	{
		EXPECT_LE(mas[i], mas[i + 1]);
	}
...
TEST(TestInsSort, SortDifferent)
{
	...
	EXPECT_NO_THROW(insertionSort(mas, mas+10000, [](int a, int b) { return a < b; }));
	for (int i = 0; i < 10000 - 1; i++)
	{
		EXPECT_LE(mas[i], mas[i + 1]);
	}
